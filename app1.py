...
# ─── STREAMLIT 網頁基礎設定 ───
st.set_page_config(page_title="創傷臨床決策", layout="wide")
st.title("🩺 臨床情境模擬：急診重症急救室（Trauma Room）")

# ─── 🔊 警報音效播放函式 ───
def play_alarm_sound():
    """在網頁前端播放真實的急診監視器警報聲"""
    # 這裡使用網路公開的醫院監視器警報音效 MP3 網址
    alarm_url = "https://www.soundjay.com/buttons/sounds/beep-11.mp3"
    
    # 利用 HTML5 的 autoplay 屬性，讓音效在程式碼被觸發時自動隱藏播放
    sound_html = f"""
...
def local_response_simulator(user_text):
    ...
    # 規則 2：醫囑明確性檢查 & 快捷鍵數據回報
        ...
        if any(kw in text for kw in ["條", "大口徑", "14g", "16g"]) and any(kw in text for kw in ["ns", "normal saline", "lr", "ringer"]):
            ...
            response_text = "（NP 冷靜回應）收到，請問要建立幾條靜脈通路？使用幾號針頭？要跑什麼輸液以及流速多少？"
        ...
        response_text = "收到，請問目前要優先聯絡哪一個科別進行緊急會診或準備手術？是一般外科(GS)、骨科(Ortho)，還是神經外科(NS)？另外病人 NPO 狀態與備血是否已確認？"
    ...
    if st.session_state.round_count >= 3:
        ...
        should_trigger_alarm = True # 啟用音效觸發開關

    return response_text, should_trigger_alarm


# ─── 住院醫師（使用者）輸入區 ───
if user_input := st.chat_input("請輸入緊急處置醫囑..."):
