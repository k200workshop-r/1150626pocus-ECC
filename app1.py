...
def call_gemini_trauma_api(user_message: str) -> NurseResponse:
    ...
    # 💡 核心校正點 2：要求模型嚴格回傳 response_text 欄位
...
if st.session_state.messages and st.session_state.messages[-1]["role"] == "user":
    ...
    with st.chat_message("model"):
        with st.spinner("資深重症 NP 回應中..."):
            try:
                ai_output = call_gemini_trauma_api(last_user_message)
                nurse_talk = ai_output.response_text
                img_url = ai_output.image_url
                img_caption = ai_output.image_caption
            except Exception as e:
                # 防當機安全機制：如果 API 真的漏了欄位，這裡會攔截，不讓網頁爆掉
                nurse_talk = f"⚠️ 護理師正忙於急救建立點滴通路中，請重新下達明確指令。（系統提示：{str(e)}）"
                img_url, img_caption = None, None

        # 雙重防線：第 3 輪休克惡化施壓
        if st.session_state.round_count >= 3 and "輸血" not in last_user_message and "mtp" not in last_user_message.lower() and "fluid" not in last_user_message.lower():
